...
lv_fs_res_t lv_ufs_dir_read(void * dir_p, char * fn)
{
    ...
    if(ufs_dir_p->last_ent != NULL) {
        AsciiStrCpyS(fn, 512, ufs_dir_p->last_ent->fn_d);
    } else {
        fn[0] = '\0';
    }
...
static lv_ufs_ent_t * lv_ufs_ent_get(const char * fn)
{
    ...
    LL_READ(file_ll, fp) {
        if((int) AsciiStrCmp(fp->fn_d, fn) == 0) {
            return fp;
        }
    }
...
static lv_ufs_ent_t * lv_ufs_ent_new(const char * fn)
{
    ...
    new_ent->fn_d = lv_mem_alloc((size_t) AsciiStrLen(fn)  + 1); /*Save the name*/
    lv_mem_assert(new_ent->fn_d);
    if(new_ent->fn_d == NULL) return NULL;

    AsciiStrCpyS(new_ent->fn_d, 256, fn);
    new_ent->data_d = NULL;
    new_ent->size = 0;
    new_ent->oc = 0;
