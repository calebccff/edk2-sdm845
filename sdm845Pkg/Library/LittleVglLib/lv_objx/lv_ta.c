...
void lv_ta_add_char(lv_obj_t * ta, uint32_t c)
{
    ...
    if(ext->pwd_mode != 0) {
        ...
        ext->pwd_tmp = lv_mem_realloc(ext->pwd_tmp, (size_t) AsciiStrLen(ext->pwd_tmp) + 2);  /*+2: the new char + \0 */
...
void lv_ta_add_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(ext->pwd_mode != 0) {
        ext->pwd_tmp = lv_mem_realloc(ext->pwd_tmp, (size_t) AsciiStrLen(ext->pwd_tmp) + (size_t) AsciiStrLen(txt) + 1);
        lv_mem_assert(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
...
#endif
        ext->pwd_tmp = lv_mem_realloc(ext->pwd_tmp, (size_t) AsciiStrLen(ext->pwd_tmp) + 1);
        lv_mem_assert(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
    }
...
void lv_ta_set_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(ext->pwd_mode != 0) {
        ext->pwd_tmp = lv_mem_realloc(ext->pwd_tmp, (size_t) AsciiStrLen(txt) + 1);
        lv_mem_assert(ext->pwd_tmp);
        if(ext->pwd_tmp == NULL) return;
        AsciiStrCpyS(ext->pwd_tmp, AsciiStrLen(txt) + 1, txt);
...
void lv_ta_set_pwd_mode(lv_obj_t * ta, bool pwd_en)
{
    ...
    if(ext->pwd_mode == 0 && pwd_en != false) {
        ...
        uint16_t len = (size_t) AsciiStrLen(txt);
