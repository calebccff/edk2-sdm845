...
void lv_label_set_text(lv_obj_t * label, const char * text)
{
    ...
    if(ext->text == text) {
        ...
        ext->text = lv_mem_realloc(ext->text, (size_t) AsciiStrLen(ext->text) + 1);
        lv_mem_assert(ext->text);
        if(ext->text == NULL) return;
    } else {
        /*Allocate space for the new text*/
        uint32_t len = (size_t) AsciiStrLen(text) + 1;
        ...
        AsciiStrCpyS(ext->text, AsciiStrLen(text) + 1, text);
...
void lv_label_ins_text(lv_obj_t * label, uint32_t pos,  const char * txt)
{
    ...
    /*Allocate space for the new text*/
    uint32_t old_len = (size_t) AsciiStrLen(ext->text);
    uint32_t ins_len = (size_t) AsciiStrLen(txt);
