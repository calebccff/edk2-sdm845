...
lv_obj_t * lv_tabview_add_tab(lv_obj_t * tabview, const char * name)
{
    ...
    if((name[0] & LV_BTNM_CTRL_MASK) == LV_BTNM_CTRL_CODE) { /*If control byte presented let is*/
        name_dm = lv_mem_alloc((size_t) AsciiStrLen(name) + 1); /*+1 for the the closing '\0' */
        lv_mem_assert(name_dm);
        if(name_dm == NULL) return NULL;
        AsciiStrCpyS(name_dm, AsciiStrLen(name) + 1, name);
    } else { /*Set a no long press control byte is not presented*/
        name_dm = lv_mem_alloc((size_t) AsciiStrLen(name) + 2); /*+1 for the the closing '\0' and +1 for the control byte */
        lv_mem_assert(name_dm);
        if(name_dm == NULL) return NULL;
        name_dm[0] = '\221';
        AsciiStrCpyS(&name_dm[1], AsciiStrLen(name) + 1, name);
    }
...
static lv_res_t tab_btnm_action(lv_obj_t * tab_btnm, const char * tab_name)
{
    ...
    while(tabs_map[i][0] != '\0') {
        if((int) AsciiStrCmp(&tabs_map[i][1], tab_name) == 0) break;   /*[1] to skip the control byte*/
        i++;
    }
